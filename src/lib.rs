...
impl<Arena, T> Component<Arena, T> {
    ...
    #[inline]
    pub fn get(&self, id: impl ValidId<Arena = Arena>) -> Option<&T> {
        self.values.get(id.id().untyped)
    }

    #[inline]
    pub fn get_mut(&mut self, id: impl ValidId<Arena = Arena>) -> Option<&mut T> {
...
impl<Arena, T> Component<Arena, Option<T>> {
    #[inline]
    pub fn remove(&mut self, id: impl ValidId<Arena = Arena>) -> Option<T> {
